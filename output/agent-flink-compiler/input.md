---
title: "从 Agent<br>到 Flink 到 Compiler<br>我发现的一些意外关联"
subtitle: 从图结构到抽象层次，一次跨越三个时代的技术观察
---
...
> "你做过数据中台、用过 Spark 和 Flink，又在做 Agent，你正好**站在两个时代的缝隙上**——既见过旧世界的武器，又能给新用户（LLM）打造接口。"
...
所以如果你做 Agent、遇到一个具体工程问题、花一个下午去搜 Chandy-Lamport、Flink backpressure、Edward Lee 的 typed dataflow、Joe Armstrong 的 Erlang、Volcano optimizer、Ptolemy II、Unix OS 设计——**大概率能在老文献里找到一个比 Agent 圈的帖子质量高 10 倍的答案**。

### 一个心法

不是要每个工程师都变成 Leslie Lamport。没人读得完所有经典。
